...
const Home = ({navigation}) => {
  ...
  const updateSocketId = async (socketId) => {
    console.log('0333', user.fbId)
    console.log('0111',socketId)
    try {
      await api.put(`/socket/${user.fbId}`, {socketId});
  ...
  const updateLocation = async (coordinates) => {
    try {
      await api.put(`/location/${user.fbId}`, {
        coordinates,
